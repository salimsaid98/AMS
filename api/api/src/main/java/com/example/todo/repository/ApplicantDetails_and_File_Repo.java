...
public interface ApplicantDetails_and_File_Repo extends JpaRepository<ApplicantDetails_and_File,Long>{
    ...
    @Query(value = " SELECT f.*, af.file_name FROM file_table f JOIN applicant_details_and_File_table af ON f.fileID = af.fileID JOIN applicant_details_table a ON a.applicantID = af.applicantID WHERE a.applicantID =  ?1", nativeQuery = true)
