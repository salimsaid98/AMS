...
@Component({
  selector: 'app-approved-applicant-admin',
  templateUrl: './approved-applicant-admin.component.html',
  styleUrls: ['./approved-applicant-admin.component.css']
})
export class ApprovedApplicantAdminComponent {
...
username:any
  ngOnInit(): void {
    ...
    this.getAllApplicantStatusIsApproved()
    this.countAllApplicantPendingByUser(this.username)
  }
  isLoading: boolean = true; // Flag to track loading state
  currentIndex = 0;
  getAllApplicantStatusIsApproved() {
    this.aplicantStatuServices.getAllApplicantStatusIsApproved().subscribe(
      applicant_response => {
