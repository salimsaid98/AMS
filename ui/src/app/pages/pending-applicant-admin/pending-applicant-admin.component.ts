...
@Component({
  selector: 'app-pending-applicant-admin',
  templateUrl: './pending-applicant-admin.component.html',
  styleUrls: ['./pending-applicant-admin.component.css']
})
export class PendingApplicantAdminComponent {
  displayedColumns: string[] = ['index','registered_date','country_to_visit', 'applicantfull_name', 'applicantemail_address','applicantphone_number','registered_by','status','view'];
...
username:any
  ngOnInit(): void {
    ...
    this.getAllApplicantStatusIsPending()
    this.countAllApplicantPendingByUser(this.username)
  }
  isLoading: boolean = true; // Flag to track loading state
  currentIndex = 0;
  getAllApplicantStatusIsPending() {
    this.aplicantStatuServices.getAllApplicantStatusIsPending().subscribe(
      applicant_response => {
